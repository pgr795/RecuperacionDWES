...
function insertInvoice($conexion,$invoiceId,$customerId,$fecha){
	try{
		...
		$insert="INSERT INTO invoice VALUES ('$invoiceId','$customerId','$fecha','0',null,null)";
...
function actualizarFechaHora($conexion,$invoiceId,$fecha,$hora){
	try{
		$conexion->beginTransaction();
		$updates= "UPDATE invoice SET Fecha_Transaccion='$fecha' WHERE InvoiceId='$invoiceId'";
		$conexion->exec($updates);
		$conexion->commit();
		
		$conexion->beginTransaction();
		$updates= "UPDATE invoice SET Hora_Transaccion='$hora' WHERE InvoiceId='$invoiceId'";
		$conexion->exec($updates);
		$conexion->commit();
	}		
	catch(PDOException $e){
		echo "Error actualizarFechaHora()"."<br>";
		echo $e->getMessage();
		$conexion->rollback();	
		$error=1;
		return $error;
